...
import { getShippingDataFromRedis, saveAnalyticsToRedis } from './redis'
...
export async function computeAllAnalytics(sessionId: string, filters?: FilterParams): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    if (filteredData.length === 0) {
      ...
      await Promise.all([
        saveAnalyticsToRedis(sessionId, `weekly-summary:${filterKey}`, []),
        saveAnalyticsToRedis(sessionId, `ndr-weekly:${filterKey}`, []),
        saveAnalyticsToRedis(sessionId, `state-performance:${filterKey}`, []),
        saveAnalyticsToRedis(sessionId, `category-share:${filterKey}`, []),
        saveAnalyticsToRedis(sessionId, `cancellation-tracker:${filterKey}`, []),
        saveAnalyticsToRedis(sessionId, `channel-share:${filterKey}`, []),
        saveAnalyticsToRedis(sessionId, `payment-method:${filterKey}`, []),
        saveAnalyticsToRedis(sessionId, `product-analysis:${filterKey}`, []),
      ])
      return { success: true }
    }
    ...
    // Save to Redis with filter key
    const filterKey = filters ? JSON.stringify(filters) : 'no-filters'
    await Promise.all([
      saveAnalyticsToRedis(sessionId, `weekly-summary:${filterKey}`, weeklySummary),
      saveAnalyticsToRedis(sessionId, `ndr-weekly:${filterKey}`, ndrWeekly),
      saveAnalyticsToRedis(sessionId, `state-performance:${filterKey}`, statePerformance),
      saveAnalyticsToRedis(sessionId, `category-share:${filterKey}`, categoryShare),
      saveAnalyticsToRedis(sessionId, `cancellation-tracker:${filterKey}`, cancellationTracker),
      saveAnalyticsToRedis(sessionId, `channel-share:${filterKey}`, channelShare),
      saveAnalyticsToRedis(sessionId, `payment-method:${filterKey}`, paymentMethod),
      saveAnalyticsToRedis(sessionId, `product-analysis:${filterKey}`, productAnalysis),
    ])
