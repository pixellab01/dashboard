 ...
 * 
 * Get product analysis analytics from Redis (with optional filters)
...
export async function GET(request: NextRequest) {
  try {
    ...
    let data = await getAnalyticsFromRedis(sessionId, `product-analysis:${filterKey}`)
    ...
    if (!data) {
      ...
      data = await getAnalyticsFromRedis(sessionId, `product-analysis:${filterKey}`)
    }
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to fetch product analysis',
      },
      { status: 500 }
    )
  }
