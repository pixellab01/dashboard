 ...
 * 
 * Get channel share analytics from Redis (with optional filters)
...
export async function GET(request: NextRequest) {
  try {
    ...
    let data = await getAnalyticsFromRedis(sessionId, `channel-share:${filterKey}`)
    ...
    if (!data) {
      ...
      data = await getAnalyticsFromRedis(sessionId, `channel-share:${filterKey}`)
    }
    ...
    console.error('Error fetching channel share:', error)
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to fetch channel share',
      },
      { status: 500 }
    )
  }
