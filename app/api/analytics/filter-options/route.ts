 ...
 * 
 * Get unique values for filter dropdowns from Redis data
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Debug logging - Check sample records for Product Name field
    if (data.length > 0) {
    ...
    // Debug logging - Check sample records for Status fields
    if (data.length > 0) {
      const sampleRecord = data[0]
