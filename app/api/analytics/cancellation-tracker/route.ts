 ...
 * 
 * Get cancellation tracker analytics from Redis (with optional filters)
...
export async function GET(request: NextRequest) {
  try {
    ...
    let data = await getAnalyticsFromRedis(sessionId, `cancellation-tracker:${filterKey}`)
    ...
    if (!data) {
      ...
      data = await getAnalyticsFromRedis(sessionId, `cancellation-tracker:${filterKey}`)
    }
    ...
    console.error('Error fetching cancellation tracker:', error)
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to fetch cancellation tracker',
      },
      { status: 500 }
    )
  }
