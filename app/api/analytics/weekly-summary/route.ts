 ...
 * 
 * Get weekly summary analytics from Redis (with optional filters)
...
export async function GET(request: NextRequest) {
  try {
    ...
    const analyticsKey = `weekly-summary:${filterKey}`

    let data = await getAnalyticsFromRedis(sessionId, analyticsKey)

    // If not found, compute with filters
    if (!data) {
      ...
      data = await getAnalyticsFromRedis(sessionId, analyticsKey)
    }

    // Return empty array if still no data (filters may have resulted in no matches)
    if (!data) {
      return NextResponse.json(
        {
          success: true,
