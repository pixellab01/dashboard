 ...
 * 
 * Get NDR weekly analytics from Redis (with optional filters)
...
export async function GET(request: NextRequest) {
  try {
    ...
    let data = await getAnalyticsFromRedis(sessionId, `ndr-weekly:${filterKey}`)
    ...
    if (!data) {
      ...
      data = await getAnalyticsFromRedis(sessionId, `ndr-weekly:${filterKey}`)
    }
    ...
    console.error('Error fetching NDR weekly:', error)
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Failed to fetch NDR weekly',
      },
      { status: 500 }
    )
  }
