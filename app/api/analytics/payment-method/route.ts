 ...
 * 
 * Get payment method analytics from Redis (with optional filters)
...
export async function GET(request: NextRequest) {
  try {
    ...
    let data = await getAnalyticsFromRedis(sessionId, `payment-method:${filterKey}`)
    ...
    if (!data) {
      ...
      data = await getAnalyticsFromRedis(sessionId, `payment-method:${filterKey}`)
    }
