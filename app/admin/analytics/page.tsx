...
export default function AnalyticsDashboard() {
  ...
  const fetchTTLInfo = async (currentSessionId: string) => {
    try {
      const response = await fetch(`/api/analytics/ttl-info?sessionId=${currentSessionId}`)
      if (response.ok) {
  ...
  const fetchFilterOptions = async (currentSessionId: string) => {
    try {
      const response = await fetch(`/api/analytics/filter-options?sessionId=${currentSessionId}`)
      const data = await response.json()
      
      if (data.success) {
        setAvailableChannels(data.channels || [])
        setAvailableSkus(data.skus || [])
        setAvailableSkusTop10(data.skusTop10 || [])
        setAvailableProductNames(data.productNames || [])
        setAvailableProductNamesTop10(data.productNamesTop10 || [])
        setAvailableStatuses(data.statuses || [])
        
        // Debug logging
        console.log('Filter options loaded:', {
          skusTop10Count: data.skusTop10?.length || 0,
          productNamesTop10Count: data.productNamesTop10?.length || 0,
          skusTop10: data.skusTop10,
          productNamesTop10: data.productNamesTop10,
          allProductNamesCount: data.productNames?.length || 0,
          firstFewProductNames: data.productNames?.slice(0, 5),
        })
        
        // Additional validation
        if (!data.productNamesTop10 || data.productNamesTop10.length === 0) {
          console.warn('⚠️ No top 10 product names found! Check API response:', {
            hasProductNames: !!data.productNames,
            productNamesCount: data.productNames?.length || 0,
            productNamesTop10Exists: !!data.productNamesTop10,
            productNamesTop10Length: data.productNamesTop10?.length || 0,
          })
        }
      }
    } catch (error) {
  ...
  const fetchAllData = async (currentSessionId: string, currentFilters: FilterState) => {
    ...
    try {
      ...
      // Fetch all analytics data with sessionId and filters
      const [
        weeklyRes,
        ndrRes,
        stateRes,
        ...
        fetch(`/api/analytics/weekly-summary?${queryString}`),
        fetch(`/api/analytics/ndr-weekly?${queryString}`),
        fetch(`/api/analytics/state-performance?${queryString}`),
        fetch(`/api/analytics/category-share?${queryString}`),
        fetch(`/api/analytics/cancellation-tracker?${queryString}`),
        fetch(`/api/analytics/channel-share?${queryString}`),
        fetch(`/api/analytics/payment-method?${queryString}`),
        fetch(`/api/analytics/summary-metrics?${queryString}`),
        fetch(`/api/analytics/order-statuses?${queryString}`),
        fetch(`/api/analytics/payment-method-outcome?${queryString}`),
        fetch(`/api/analytics/product-analysis?${queryString}`),
      ])

      const [weeklyData, ndrData, stateData, categoryData, cancellationData, channelData, paymentData, summaryData, statusesData, paymentOutcomeData, productAnalysisData] = await Promise.all([
        weeklyRes.json(),
        ndrRes.json(),
        stateRes.json(),
        categoryRes.json(),
        cancellationRes.json(),
        channelRes.json(),
        paymentRes.json(),
        summaryRes.json(),
        statusesRes.json(),
        paymentOutcomeRes.json(),
        productAnalysisRes.json(),
      ])
      ...
      if (needsComputation && currentSessionId) {
        ...
        fetch('/api/analytics/compute', {
          ...
          body: JSON.stringify({ sessionId: currentSessionId, filters: currentFilters }),
        })
        .then(async (computeRes) => {
          const computeResult = await computeRes.json()
          if (computeRes.ok && computeResult.success) {
  ...
  useEffect(() => {
    const fetchRawShippingData = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/analytics/raw-shipping?${filterParams.toString()}`)
