...
import { getAnalyticsByFilters, buildAnalyticsKey, saveAnalyticsToRedis, getShippingDataFromRedis, isSourceDataValid } from '@/lib/redis'
...
export async function GET(request: NextRequest) {
  try {
    ...
    let metrics = await getAnalyticsByFilters(sessionId, 'summary-metrics', filterObj)
    ...
    if (!metrics) {
      ...
      const filters: FilterParams = filterObj
    ...
    filteredData.forEach((record: any) => {
      ...
      // Cache the result
      const keyPart = buildAnalyticsKey(sessionId, 'summary-metrics', filterObj).replace(`analytics:${sessionId}:`, '')
      await saveAnalyticsToRedis(sessionId, keyPart, metrics)
    }
